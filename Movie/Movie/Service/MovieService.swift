...
final class MovieService {
    ...
    func fetchMovies(targetDt: Int = 20250101) -> AnyPublisher<[DailyBoxOffice], NetworkError> {
        guard var components = URLComponents(string: "http://www.kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchDailyBoxOfficeList") else {
    ...
    func fetchMovieInfo(movieCd: String) -> AnyPublisher<MovieInfo, NetworkError> {
        guard var components = URLComponents(string: "http://www.kobis.or.kr/kobisopenapi/webservice/rest/movie/searchMovieInfo") else {
            return Fail(error: NetworkError.invalidURL)
                .eraseToAnyPublisher()
        }
        ...
        return URLSession.shared.dataTaskPublisher(for: url)
            .mapError { _ in NetworkError.invalidResponse }
            .map(\.data)
            .decode(type: MovieInfoResponse.self, decoder: JSONDecoder())
            .mapError { NetworkError.decodingError($0) }
            .map(\.movieInfoResult.movieInfo)
            .receive(on: DispatchQueue.main)
            .eraseToAnyPublisher()
    }
}
